...
export default function InscriptionStep1Profil({ formData, setFormData, onNext, errors }: Props) {
  // LOGIQUE DE VALIDATION (CAC40 Mindset : Précision et Rigueur)
  const isFormValid = formData.firstName.trim().length >= 3 && formData.gender !== "" && formData.lookingFor !== "";
  ...
  const interactiveClasses =
    "hover:border-[hsl(var(--gold))] focus:border-[hsl(var(--gold))] focus:ring-0 focus:ring-offset-0 transition-all duration-300";
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
      <div className="text-center mb-10">
        ...
        <p className="text-muted-foreground text-xl">Ces informations sont essentielles pour certifier votre compte.</p>
      </div>
      ...
      <div>
        ...
        />
        <p className="text-xs text-muted-foreground mt-2 uppercase tracking-widest font-bold">3 caractères minimum</p>
      </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <button
                ...
                className={cn(
                  "h-14 px-3 rounded-xl text-lg md:text-xl font-medium border-2 transition-all duration-300",
                  formData.gender === opt.value
                    ? "border-[#1B2333] bg-[#1B2333] text-white shadow-md scale-[1.02]"
      ...
      {/* BOUTON CONTINUER : VERROUILLÉ JUSQU'À VALIDATION COMPLÈTE */}
      <Button
        type="button"
        onClick={onNext}
        disabled={!isFormValid}
        className={cn(
          "w-full h-14 text-xl rounded-xl mt-4 shadow-lg transition-all flex items-center justify-center gap-2",
          isFormValid
            ? "bg-[#1B2333] text-white hover:bg-[#1B2333]/90 hover:scale-[1.01] active:scale-[0.99]"
