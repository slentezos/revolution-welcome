      ...
      <DialogContent className="max-w-[98vw] xl:max-w-[1300px] h-[94vh] p-0 gap-0 overflow-hidden bg-background rounded-xl border-none shadow-[var(--shadow-luxury)]">
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex-1 overflow-y-auto">
              ...
              <div className="flex flex-col lg:flex-row min-h-full">
                <div className="lg:w-[50%] flex flex-col bg-secondary/50 p-4 lg:p-5">
                  ...
                  <div className="relative flex-1 min-h-[400px] rounded-xl overflow-hidden group/media">
                    ...
                    <button
                      onClick={goToPrev}
                      className="absolute left-3 top-1/2 -translate-y-1/2 flex items-center gap-2 px-4 py-2.5 rounded-full bg-background/90 hover:bg-background shadow-md transition-all duration-200 group-hover/media:opacity-100"
                    >
                    ...
                    <button
                      onClick={goToNext}
                      className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2 px-4 py-2.5 rounded-full bg-background/90 hover:bg-background shadow-md transition-all duration-200 group-hover/media:opacity-100"
                    >
            ...
            {/* ── QUIZ TAB (Optimized) ── */}
            {activeTab === "quiz" && (
              <div className="flex flex-col min-h-full bg-[hsl(var(--cream))] p-6 lg:p-10 pb-16">
                <div className="flex items-center gap-3 mb-10">
                  <span className="w-8 h-[2px] bg-[hsl(var(--gold))]" />
                  <h3 className="font-heading text-3xl font-semibold text-foreground">Quiz des 3 préférences</h3>
                  <span className="text-muted-foreground text-xl">— Ce que {match.name} apprécie</span>
                </div>

                {/* Grille optimisée en 3 colonnes pour éviter le vide vertical */}
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {quizCategories.map((cat, idx) => (
                    <div
                      key={idx}
                      className="bg-background rounded-2xl p-6 flex flex-col border border-border/40 shadow-sm hover:shadow-md transition-all duration-300 h-auto"
                    >
                      ...
                      {/* Affichage en "Pills" horizontaux pour mieux remplir la carte */}
                      <div className="flex flex-wrap gap-2">
                        {cat.items?.map((item, itemIdx) => (
                          <span
                            key={itemIdx}
                            className="px-4 py-2 bg-secondary/60 rounded-full text-foreground font-medium border border-border/20 text-lg transition-colors hover:bg-secondary"
                          >
                            {item}
                          </span>
                        ))}
                ...
                <div className="grid lg:grid-cols-2 gap-0">
                  <div className="bg-primary/[0.04] p-8 lg:p-10">
                    <h3 className="font-heading text-2xl font-semibold text-foreground mb-5 flex items-center gap-2">
                      <span className="w-6 h-[2px] bg-[hsl(var(--gold))]" /> Points forts
                    </h3>
                  ...
                  <div className="bg-primary/[0.04] p-8 lg:p-10 lg:bg-transparent border-t lg:border-t-0 lg:border-l border-border/10">
                    <h3 className="font-heading text-2xl font-semibold text-foreground mb-5 flex items-center gap-2">
                      <span className="w-6 h-[2px] bg-muted-foreground/40" /> Points faibles
                    </h3>
                    <div className="space-y-3">
          ...
          {!hideActions && (
            <div className="border-t border-border/20 px-6 lg:px-10 py-4 flex items-center justify-between bg-primary flex-shrink-0 z-20">
              ...
              <div className="flex items-center gap-3 ml-auto">
                ...
                >
                  <Clock className="h-4 w-4" /> Décider plus tard
                </Button>
                <Button
