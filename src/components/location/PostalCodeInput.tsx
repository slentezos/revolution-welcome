...
export default function PostalCodeInput({ className = "", variant = "hero" }: PostalCodeInputProps) {
  ...
  const handleChange = (value: string) => {
    ...
    if (cleaned.length === 5) {
      ...
      if (info) setLocationInfo(info);else
  ...
  return (
    <div className={className}>
      <div className={`flex items-center gap-0 rounded-sm overflow-hidden shadow-elevated ${isHero ? "max-w-lg mx-auto" : "max-w-md"}`}>
        <input
          type="text"
          inputMode="numeric"
          maxLength={5}
      ...
      {/* Helper text */}
      <p className={`text-sm mt-3 flex items-center gap-1.5 ${isHero ? "text-muted-foreground justify-center" : "text-primary-foreground/60"}`}>
